$(function() {
    ...
    if ($('.chart-circle').length) {
        $('.chart-circle').each(function() {
            var $this = $(this);
            $this.circleProgress({ fill: { color: 'indigo' }, size: $this.height(), startAngle: -Math.PI / 4 * 2, emptyFill: '#F4F4F4', lineCap: 'round' });
        });
    }
    $('.bh_income').sparkline('html', { type: 'bar', height: '30px', barColor: '#6435c9', barWidth: 5, });
    $('.bh_traffic').sparkline('html', { type: 'bar', height: '30px', barColor: '#e03997', barWidth: 5, });
    var alterClass = function() { var ww = document.body.clientWidth; if (ww < 1530) { $('body').addClass('close_rightbar'); } else if (ww >= 1531) { $('body').removeClass('close_rightbar'); }; };
    $(window).resize(function() { alterClass(); });
    alterClass();
    $('a.right_tab').on('click', function() { $('body').toggleClass('right_tb_toggle'); });
...
$(document).ready(function() {
    $('.star').on('click', function() { $(this).toggleClass('star-checked'); });
...
$(function() {
    ...
    $('.menu_toggle').on('click', function() { $('body').toggleClass('offcanvas-active'); });
    $('.chat_list_btn').on('click', function() { $('.chat_list').toggleClass('open'); });
    $('.menu_option').on('click', function() {
        $('.metismenu').toggleClass('grid');
        $('.menu_option').toggleClass('active');
    });
    $('.user_btn').on('click', function() { $('.user_div').toggleClass('open'); });
    $('.right_chat li a, .user_chatbody .chat_close').on('click', function() { $('.user_chatbody').toggleClass('open'); });
    $('a.settingbar').on('click', function() { $('.right_sidebar').toggleClass('open'); });
    $('a.theme_btn').on('click', function() { $('.theme_div').toggleClass('open'); });
    $('.page').on('click', function() {
        $('.theme_div, .right_sidebar').removeClass('open');
        $('.user_div').removeClass('open');
    });
    $('.theme_switch').on('click', function() { $('body').toggleClass('theme-dark'); });
});
$(function() {
    "use strict";
    $('.arrow_option input:radio').click(function() {
        var others = $("[name='" + this.name + "']").map(function() { return this.value }).get().join(" ")
        console.log(others)
